...
- Generate missing blog frontmatter summaries with `npm run content:generate-ai-summaries`; it uses `gpt-5.4-mini`, skips posts that already have `ai_summary`, and supports resumable parallel runs. The pre-commit hook runs it with `--staged-new` and stages generated summaries for newly added posts. Use `--force` only when intentionally regenerating every summary.
