...
- Generate missing blog frontmatter summaries with `npm run content:generate-ai-summaries`; it uses `gpt-5.4-mini`, skips posts that already have `ai_summary`, and supports resumable parallel runs. Use `--force` only when intentionally regenerating every summary.
- After publishing or materially updating a post, notify participating search engines with `npm run content:notify-indexnow -- <post-slug>`. Pass multiple slugs or full `nickang.com` URLs to submit them together.
